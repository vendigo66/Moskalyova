...
def count_letters(text):
    ...
    for letter in small_text:
        if letter.isalpha():
            if letter in letters_quantity:
                letters_quantity[letter]+=1
            else:
                letters_quantity[letter]=1
...
def calculate_frequency(letters_quantity):
    total_letters = sum(letters_quantity.values())

    letter_frequency = {}
    for current_letter, count in letters_quantity.items():
